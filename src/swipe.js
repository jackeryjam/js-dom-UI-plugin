module.exports = function (dom, clickCb, cb, option) {
    var opt = {}
    opt.domWidth = option && option.domWidth || window.innerWidth;
    opt.appendWidth = option && option.appendWidth || 60;
    var currentPosition = 0; //记录当前页面位置

    var app = {
        //页面平移
        transform: function (translate) {
            if (translate < -opt.appendWidth || translate > 0) return
        ...
        bindTouchEvent: function () {
            ...
            var maxWidth = - (opt.domWidth + opt.appendWidth);
            var startX, startY;
            ...
            dom.addEventListener("touchmove", function (e) {
                if (first) {
                    ...
                    if (Math.abs(deltaX) > Math.abs(deltaY)) {
                        moveLength = deltaX < opt.appendWidth ? deltaX : opt.appendWidth;
            ...
            dom.addEventListener("touchend", function (e) {
                if(click && clickCb) clickCb()
                // e.preventDefault();
                //发生了滑动，并且当前滑动事件未结束
                if (isMove && !isTouchEnd) {
                    isTouchEnd = true;
                    viewport.style.webkitTransition = "0.3s ease -webkit-transform";
                    currentPosition = currentPosition > -opt.appendWidth / 2 ? 0 : - opt.appendWidth;
